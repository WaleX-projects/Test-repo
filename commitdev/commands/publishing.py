...
# (Mocking these out based on original script structure)
...
def render_image_preview(img_source, img_name, kb_size, is_url):
    """Renders a pixelated preview of an image in the terminal."""
    try:
...
# ──────────────────────────────────────────────────────────
# WIZARD ROUTINES (Extracted from nested structure)
# ──────────────────────────────────────────────────────────

async def run_edit_content_sequence(ws, post_id, staged_platforms, posts_by_platform):
    """Handles the local editing and synchronization of draft text."""
...
async def run_regenerate_content_sequence(ws, post_id, staged_platforms, posts_by_platform):
    """Triggers the AI worker to regenerate post drafts."""
    if not staged_platforms:
...
async def run_image_manager_sequence(ws, post_id, attached_images):
    """Handles adding, viewing, and clearing remote/local images."""
...
async def run_platforms_manager_sequence(staged_platforms, posts_by_platform, commit_msg):
    """Handles enabling or disabling publishing endpoints."""
...
async def handle_publishing_pipeline(ws, payload):
    ...
    while True:
        ...
        if "Edit" in selection:
            await run_edit_content_sequence(ws, post_id, staged_platforms, posts_by_platform)
        elif " Regenerate" in selection:
            await run_regenerate_content_sequence(ws, post_id, staged_platforms, posts_by_platform)
        elif "Manage Media" in selection:
            await run_image_manager_sequence(ws, post_id, attached_images)
        elif " Toggle" in selection:
            await run_platforms_manager_sequence(staged_platforms, posts_by_platform, commit_msg)
